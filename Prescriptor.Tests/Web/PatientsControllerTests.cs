...
namespace Prescriptor.Tests.Web
{
    public class PatientsControllerTests
    {
        ...
        public async Task Index_ShouldContain_Entities()
        {
            ...
            var model = (List<Patient>)((ViewResult)result).Model;
        ...
        [Theory(DisplayName = "Details Should Contain Patient Entity")]
        ...
        public void Delete_ShouldContain_PatientEntity(int id)
        {
            var context = GetContext();
            var controller = new PatientsController(context);
            var resultTask = controller.Delete(id);
                resultTask.Wait();

                Assert.NotNull(resultTask);

                var result = resultTask.Result;
                var model = (Patient)((ViewResult)result).Model;
